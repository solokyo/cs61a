def flatten(lst):
    ...
    flattened_list = []
    for element in lst:
        if isinstance(element, list): flattened_list.extend(flatten(element))
...
def my_reduce(combiner, seq):
    ...
    # return total
    return (lambda f: lambda acc, seq: f(f, acc, seq))(lambda self, acc, seq: acc if not seq else self(self, combiner(acc, seq[0]), seq[1:]))(seq[0], seq[1:])
...
def closer_city(x, y, a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(a) if distance(make_city('', x, y), a) < distance(make_city('', x, y), b) else get_name(b)
...
def count_palindromes(L):
    ...
    2
    """
    # return len(list(filter(lambda s: s == s[::-1], map(str.lower, L))))
    return len(my_filter(lambda s: s == s[::-1], my_map(str.lower, L)))
