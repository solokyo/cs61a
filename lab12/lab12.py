...
def floor_div(expr: 'Pair'):
    ...
    if expr.rest is nil:
        return expr.first
    expr.first = expr.first // expr.rest.first
    expr.rest = expr.rest.rest
    return floor_div(expr)


def eval_and(operands: 'Pair'):
    """
    ...
    """
    # BEGIN SOLUTION Q4
    if operands.first is False: return False
    if isinstance(operands.first, Pair):
        operands.first = calc_eval(operands.first)
        return eval_and(operands)
    if operands.rest is nil: return operands.first
    return eval_and(operands.rest)

bindings = {}


def eval_define(expr: 'Pair'):
    """
    ...
    >>> calc_eval("c")
    1
    >>> eval_define(Pair("d", Pair("and", Pair(False, Pair("1", nil)))))
    'd'
    >>> calc_eval("d")
    False
    """
    # BEGIN SOLUTION Q5
    if expr.rest.rest is not nil:
