...
def num_trees(n):
    ...
    *   *        *       *
    ...
    return factorial(2*(n-1))//(factorial(n-1)*factorial(n))
...
def trade(first, second):
    ...
    while i < len(first) or j < len(second):
        while m < n and i < len(first):
            m += first[i]
            i += 1
        while n < m and j < len(second):
            n += second[j]
            j += 1
        if m == n:
...
def shuffle(cards):
    ...
    return [elem for pairs in zip(cards[:len(cards)//2], cards[len(cards)//2:]) for elem in pairs]
...
def deep_len(lnk):
    ...
    else: return deep_len(lnk.first) + deep_len(lnk.rest)
...
def make_to_string(front, mid, back, empty_repr):
    ...
    """
    def printer(lnk):
        return empty_repr if not lnk else front + str(lnk.first) + mid + printer(lnk.rest) + back
    return printer
...
def reverse_other(t):
    ...
    def reverse_helper(t, need_reverse):
        ...
        for i in range(len(t.branches)):
            ...
            if need_reverse: child.label = new_labs[i]
