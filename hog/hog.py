...
def is_always_roll(strategy, goal=GOAL):
    ...
    for a in range(0, goal):
        for b in range(0, goal):
            if strategy(a,b) != r: return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_function(*args, **kwargs):
        return sum(original_function(*args, **kwargs) for _ in range(total_samples)) / total_samples
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    s: int = tail_points(opponent_score)
    return 0 if s >= threshold or (perfect_square(score+s) and next_perfect_square(score+s)-score >= threshold) else num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
