...
class Insect:
    ...
    def __init__(self, health:int, place:Place=None):
...
class Ant(Insect):
    ...
    def add_to(self,place:Place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            other = place.ant
            if(isinstance(other, ContainerAnt) and other.can_contain(self)):
            ...
            if(isinstance(self, ContainerAnt) and self.can_contain(other)):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while(p and not p.is_hive):
            if(d >= self.lower_bound and d <= self.upper_bound and len(p.bees) != 0): return random_bee(p.bees)
            p = p.entrance
            d += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        if self.health - amount <= 0: self._skill(self.damage + amount)
        else: self._skill(amount)
        super().reduce_health(amount)

    def _skill(self,x:int):
        for i in self.place.bees.copy():
            i.reduce_health(x)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: 'GameState'):
        if(not self.turns_to_chew and self.place.bees):
            foo = random_bee(self.place.bees)
            foo.reduce_health(foo.health)
            self.turns_to_chew = self.chewing_turns
        else:
            self.turns_to_chew = max(self.turns_to_chew-1, 0)
# END Problem 7
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target:'Bee'):
        ...
        # Not sure what the template trying to do...
        # BEGIN Problem 13 
        # if _______________:
        ...
        global cnt 
        cnt = 0
        def slow_action(target, gamestate:GameState):
            global cnt
            if cnt < 5 and not gamestate.time % 2:
                act(gamestate)
            cnt += 1
            if cnt >= 5:
                target.action=act.__get__(target)
        target.action = slow_action.__get__(target)
