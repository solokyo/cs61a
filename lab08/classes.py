...
class Card:
    ...
    def power(self, opponent_card: 'Card') -> int:
        """
        Calculate power as:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        -- The doc test is modified cause it might be worng, it discard non-highest power card --
        ...
        TA Card discards tutor from my hand to increase its own power!
        >>> print(player1.hand)
        [card1: Staff, [0, 100], card3: Staff, [100, 0]]
        >>> print(test_card.attack, test_card.defense)
        10500 10500
        """
        "*** YOUR CODE HERE ***"
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        re_add = self.attack >=0 and self.defense >= 0 and self.power(opponent_card) >= 0
        self.attack -= 1000
        self.defense -= 1000
        if re_add: player.hand.append(self)
        # You should add your implementation above this.
        if re_add:
